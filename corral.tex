...
%
\yieldcoarse instruments \texttt{yield} statements in a binary fashion: if \whoop finds that a pair of entry points cannot race, then it will instrument a \texttt{yield} only after each lock and unlock statement of this pair; else if \whoop finds that a pair of entry points might race, then it will instrument context-switches after all visible operations of this pair.
%
\yieldmr performs a finer-grained instrumentation: it instruments context-switches only after each memory region that can potentially race (regardless if a pair has not been fully verified as race-free) and after each lock and unlock. In our experiments (see \S\ref{evaluation}), \yieldmr significantly outperforms \yieldcoarse.
