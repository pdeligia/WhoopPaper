...
The candidate invariants are based on the global variables that \whoop instruments to perform lockset analysis and race checking (see Section~\ref{tech:method}). The key idea behind watchdog summarisation is that we automatically generate candidate invariants in the granularity of watched accesses. This means that for e.g. watched access \texttt{\$M.0[\$p0]} the entry point has performed a write access, but for \texttt{\$M.0[\$p1]} it has not. This reduces the false positives from reporting errors if only e.g. a field of a struct is racing, but the rest of the fields are not. An example of candidate post-conditions is the following:
...
\subsection{Optimisations for Precision}
