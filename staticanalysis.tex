...
\whoop proceeds by instrumenting each entry point call-graph for lockset analysis by replacing each call to a locking or unlocking function, with an instrumented version of the original lock and unlock functions. This means that Example~\ref{fig:smack_translation} will be instrumented as:

\begin{boogie}%[caption = Program instrumented with lock/unlock functions, label = fig:lock_instrumentation]
