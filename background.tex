...
Figure~\ref{fig:locksets} shows an example of applying lockset analysis on a concurrent program consisting of two threads $T_1$ and $T_2$, both accessing the same shared variable $A$. Thus, we have to track three locksets: the current locksets $\mathit{CLS}_1$ and $\mathit{CLS}_2$ for $T_1$ and $T_2$ respectively; and the lockset $\mathit{LS}_A$ for $A$. Initially, $\mathit{LS}_A$ contains the two locks $M$ and $N$ that the two threads acquire during execution. When $T_1$ terminates, $\mathit{LS}_A$ contains only the lock $N$, because the thread write accessed $A$ without holding $N$. Next, during the execution of $T_2$, the thread write accesses $A$ without holding $M$. Thus, $\mathit{LS}_A$ becomes the empty set. This means that there is a potential data race between the two threads on $A$ as $T_2$ does not consistently protect $A$ with at least one common with $T_1$ lock.
