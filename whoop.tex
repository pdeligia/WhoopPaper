...
To achieve scalability, SMACK uses a \emph{split} memory model that exploits an alias analysis to soundly partition memory locations into non-overlapping equivalence classes that do not alias. This has been shown to lead to more tractable verification compared with a \emph{monolithic} model where the heap is considered to be an array of bytes~\cite{rakamaric2009scalable}. The split memory model is based on \emph{memory regions}, which are maps of integers that model the heap. A benefit of using this model is that distinct memory regions denote disjoint sections of the heap. We leverage this knowledge inside \whoop to guide and optimize our lockset instrumentation and analysis, and to create a fine-grained context-switch instrumentation as discussed in \S\ref{corral}. Note that the SMACK's memory model can potentially be unsound in (typically rare) situations where programs use unaligned byte-level memory accesses. The latest release of SMACK addresses this issue, and as future work we plan to integrate the latest SMACK with \whoop to further improve soundness.
...
The candidate invariants are based on the watchdog constants that \whoop instruments for watchdog race-checking. Our candidate generation rules are purely heuristic. The key idea is to automatically generate candidates for each memory region in the granularity of watched accesses. For example, if entry point $T$ has a procedure $f$ that only writes (and not reads) the offset $z$ of memory region $\mathit{MR}$, which is associated with the write set $\hasbeenwritten{\mathit{MR}, i}$ and read set $\hasbeenread{\mathit{MR}, i}$, then when instrumenting $T$ with symbol $i$ (using the notation from \S\ref{sec:symbolicpairwise}), we might guess $\mathit{watched}_{\mathit{MR}} = z \Longrightarrow z \in \hasbeenwritten{\mathit{MR}, i}$ and $\mathit{watched}_{\mathit{MR}} = z \Longrightarrow z \notin \hasbeenread{\mathit{MR}, i}$, as postconditions for $\instrument{f}{i}$. Likewise, if $f$ releases mutex $m$, then we might guess $m \notin \currentlockset{i}$ as a postcondition.

Using watchdog-based candidates reduces the false positives from reporting errors if only e.g.\ a field in memory region $\mathit{MR}$ is racing, but the rest of the fields in $\mathit{MR}$ are not racing. To generate the candidate invariants, \whoop performs an inter-procedural static analysis on the call-graph (in Boogie IVL code) of each entry point, identifying all possible accesses to shared memory locations. To be sound, if \whoop cannot infer information for a specific access, it generates a coarse-grained candidate for the corresponding memory region, e.g.\  $z \in \hasbeenwritten{\mathit{MR}, i}$ and $z \notin \hasbeenread{\mathit{MR}, i}$ for the above example.

We emphasize that the candidate invariants generated by \whoop are just that: candidates. The tool is free to speculatively generate candidates that later turn out to be incorrect: these are simply discarded by Houdini. A consequence is that incorrect or unintended candidates generated due to bugs in \whoop cannot compromise the soundness of verification.
