...
\paragraph{Simple concurrent programming model}

A \emph{concurrent program} is described by a finite set of \emph{shared} variables, $\sharedvars$, a finite set of mutexes, $\mutexes$, and a finite set of threads, $T_1,\dots,T_n$ (for some $n > 0$).  A thread, $T$, is described by a finite set of procedures, $\procedures{T}$, and a finite set of thread-private variables, $\variables{T}$.  Thread execution commences from a designated procedure $\mainforthread{T} \in \procedures{T}$.  A procedure $P \in \procedures{T}$ is represented by a control flow graph of basic blocks.  Each basic block contains a sequence of statements, where the forms of statement are shown in Figure~\ref{TODO}.  A basic block either has a single successor or a pair of successors.  In the latter case, an \emph{exit condition} over thread-private variables determines, at runtime, the successor to which control should flow on block exit.
...
\ADComment{Need to say something clear, but intuitive rather than precise, about the semantics.  Must say what a data race is.  Must address the weak memory issue.}
