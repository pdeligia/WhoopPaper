...
To make our analysis scale while maintaining precision, and to support recursion, we use the Houdini algorithm~\cite{flanagan2001houdini} to automatically compute summaries (pre- and post-conditions and loop invariants).  Given a pool of \emph{candidate} pre-conditions, post-conditions, and loop invariants, Houdini repeatedly attempts to verify each procedure.  Initially, the entire candidate pool is considered.  If verification fails due to an incorrect candidate, this candidate is discarded.  The process repeats until a fixpoint is reached.  The (possibly empty) set of remaining candidates have been proven to hold, and can used to summarize calls and loops during further program analysis.
