...
We consider a concurrent programming model in which an unbounded number of threads execute a set of pre-defined \emph{thread templates}.  In the context of device drivers, a thread template corresponds to a driver entry point.  At any given point of execution a certain number of threads are active, each thread executing a particular template.  As in the case of drivers, where multiple distinct entry points as well as multiple instances of the same entry point may run concurrently, two distinct threads in our model may execute according to the same template or according to different templates.  Further threads may be launched at any point during execution, just as in the context of device drivers where the OS may invoke additional driver entry points.  For ease of presentation only, our model does not feature aggregate data types, pointers or dynamic memory allocation.  These \emph{are} handled by our implementation, and in Section~\ref{sec:whoopimplementation} we discuss interesting practical issues arising from the handling of a full-blown language.
