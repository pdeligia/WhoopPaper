...
We achieve this in the following manner.  Given an arbitrary symbol, $i$, we define the \emph{instrumentation} of a
...
\ADComment{Next up: describe lockset instrumentation with respect to Figure~\ref{ }.  This involves explaining what the sets $W$, $R$, $\mathit{LS}$ and $\mathit{CLS}$ are, and explaining how the instrumentation works.}

\ADComment{Finally, explain the main program that we check, which initializes the sets and runs the mains of each template.}

\ADComment{Last of all, pay attention to termination.}
